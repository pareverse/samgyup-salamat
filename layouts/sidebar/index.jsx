...
import { chakra, Flex, Grid, GridItem, Icon, Link, Text, useColorMode, useColorModeValue } from '@chakra-ui/react'
import { FiBriefcase, FiCheckCircle, FiClock, FiFileMinus, FiFileText, FiFolderMinus, FiGrid, FiLock, FiLogOut, FiMoon, FiStar, FiSun, FiTarget, FiUsers } from 'react-icons/fi'

const Sidebar = ({ isAdmin, isEmployee, isUser, isSidebarOpen, onSidebarClose }) => {
	...
	return (
		<>
			<chakra.div bg="hsla(0, 0%, 0%, 0.4)" position="fixed" top={0} left={0} h="100vh" w="full" visibility={isSidebarOpen ? 'visible' : 'hidden'} opacity={isSidebarOpen ? 1 : 0} transition="0.4s ease-in-out" zIndex={{ base: 100, lg: 99 }} onClick={onSidebarClose} />

			<chakra.aside bg="system" position="fixed" top={{ base: 0, lg: 'auto' }} left={{ base: isSidebarOpen ? 0 : -256, lg: 'auto' }} borderRight="1px solid" borderColor="border" h={{ base: 'full', lg: 'calc(100% - 72px)' }} w={256} transition="0.4s ease-in-out left" zIndex={{ base: 100, lg: 99 }}>
				<Grid templateRows="1fr auto" h="full">
					{isAdmin && (
						<GridItem p={6}>
							...
							<NextLink href="/admin/schedules">
								<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('schedules') ? 1 : 0} onClick={onSidebarClose}>
									<Flex align="center" gap={3}>
							...
							<NextLink href="/admin/payrolls">
								<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('payrolls') ? 1 : 0} onClick={onSidebarClose}>
									<Flex align="center" gap={3}>
							...
							<NextLink href="/admin/meetings">
								<Link as="span" display="block" py={2} lineHeight={6} active={router.pathname.includes('meetings') ? 1 : 0} onClick={onSidebarClose}>
									<Flex align="center" gap={3}>
					...
					{isUser && (
						<GridItem p={6}>
							<Link as="span" display="block" py={2} lineHeight={6} cursor="not-allowed" _hover={{ color: 'accent-1' }}>
					...
					<GridItem p={6}>
						<Link as="button" display="block" py={2} w="full" lineHeight={6} onClick={toggleColorMode}>
							<Flex align="center" gap={3}>
