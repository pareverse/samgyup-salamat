...
const AddLeaveModal = ({ session }) => {
	...
	return (
		<Modal
			...
			size="xl"
			toggle={(onOpen) => (
				<Button size="lg" colorScheme="brand" onClick={() => setFiles(null) || clearErrors() || reset() || onOpen()}>
					Add New
				</Button>
			)}
				...
				<Flex direction="column" gap={6}>
					...
					{watch('from') && watch('to') && (
						<FormControl>
							...
							<Input type="number" value={calcDate(watch('from'), watch('to')).total_days} size="lg" cursor="not-allowed" readOnly {...register('days')} />
						</FormControl>
					)}
...
const ViewModal = ({ users, leave }) => {
	...
	const editLeave = useMutation((data) => api.update('/leaves', leave._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('employee_leaves')
			setIsLoading(false)
			disclosure.onClose()
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Leave cancelled successfully." />
			})
		}
	})
	...
	return (
		<Modal title="Leave Information" size="xl" toggle={(onOpen) => <IconButton size="xs" icon={<FiMoreHorizontal size={12} />} onClick={onOpen} />} disclosure={disclosure}>
			<Flex direction="column" gap={6}>
