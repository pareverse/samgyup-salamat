	...
	try {
		...
		if (user) {
			if (user.password === data.password) {
				if (attendance.filter((a) => a.user === user._id.toString()).length === 0) {
					create()
				} else {
					if (attendance.filter((a) => a.user === user._id.toString() && a.date === new Date().toLocaleString('en-US', { timeZone: 'Asia/Manila' }).split(',')[0].trim()).length === 0) {
						create()
					} else {
						return res.status(401).send({ type: 'timein', message: 'You are already time in.' })
					}
				}

				const payroll = payrolls.filter((payroll) => payroll.user === user._id.toString() && !payroll.status).length
