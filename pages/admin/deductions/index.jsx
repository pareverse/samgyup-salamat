...
const EditModal = ({ deduction }) => {
	...
	const addMutation = useMutation((data) => api.update('/deductions', deduction._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('deductions')
			setIsLoading(false)
			disclosure.onClose()
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Deduction updated successfully." />
			})
		}
	})
		...
		>
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					...
					<FormControl isInvalid={errors.amount}>
						<FormLabel>Amount</FormLabel>
						<Input defaultValue={deduction.amount} type="number" size="lg" {...register('amount', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>
	...
	const deleteMutation = useMutation((data) => api.remove('/deductions', data.id), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Deduction deleted succesfully." />
			})
		}
	})
					...
					<Table
						...
						td={(deduction) => (
							<Tr key={deduction._id}>
								...
								<Td>
									<Text>{month[deduction.created.split(',')[0].split('/')[0] - 1] + ' ' + deduction.created.split(',')[0].split('/')[1] + ', ' + deduction.created.split(',')[0].split('/')[2]}</Text>
								</Td>

								<Td>
									<Text>{month[deduction.updated.split(',')[0].split('/')[0] - 1] + ' ' + deduction.updated.split(',')[0].split('/')[1] + ', ' + deduction.updated.split(',')[0].split('/')[2]}</Text>
								</Td>
