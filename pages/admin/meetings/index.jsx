	...
	const addMutation = useMutation((data) => api.create('/meetings', data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Meeting added successfully." />
			})
		}
	})
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					...
					<FormControl isInvalid={errors.schedule}>
						...
						<Input type="datetime-local" size="lg" {...register('schedule', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>
...
const EditModal = ({ meeting }) => {
	...
	const addMutation = useMutation((data) => api.update('/meetings', meeting._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('meetings')
			setIsLoading(false)
			disclosure.onClose()
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Meeting updated successfully." />
			})
		}
	})
		...
		>
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					<FormControl isInvalid={errors.title}>
						<FormLabel>Title</FormLabel>
						<Input defaultValue={meeting.title} size="lg" {...register('title', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>

					<FormControl isInvalid={errors.schedule}>
						<FormLabel>Schedule</FormLabel>
						<Input type="datetime-local" defaultValue={meeting.schedule} size="lg" {...register('schedule', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>
							...
							<Tr key={meeting._id}>
								...
								<Td>
									<Flex justify="end" align="center" gap={3}>
										...
										<IconButton variant="tinted" size="xs" colorScheme="red" icon={<FiTrash2 size={12} />} onClick={() => onDelete(meeting._id)} />
									</Flex>
								</Td>
							</Tr>
						)}
