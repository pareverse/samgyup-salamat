...
const EditModal = ({ position }) => {
	...
	const addMutation = useMutation((data) => api.update('/positions', position._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('positions')
			setIsLoading(false)
			disclosure.onClose()
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Position updated successfully." />
			})
		}
	})
	...
	const deleteMutation = useMutation((data) => api.remove('/positions', data.id), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Position deleted succesfully." />
			})
		}
	})
			...
			<Flex direction="column" gap={6}>
				...
				<Card>
					<Table
						...
						th={['ID', 'Title', 'Rate Per Hour', 'Created', 'Updated', '']}
						td={(position) => (
							<Tr key={position._id}>
								<Td>
									<Badge variant="tinted" colorScheme="brand">
										#{position._id.slice(15, 30).toUpperCase()}
									</Badge>
								</Td>
								...
								<Td>
									<Text>{month[position.created.split(',')[0].split('/')[0] - 1] + ' ' + position.created.split(',')[0].split('/')[1] + ', ' + position.created.split(',')[0].split('/')[2]}</Text>
								</Td>

								<Td>
									<Text>{month[position.updated.split(',')[0].split('/')[0] - 1] + ' ' + position.updated.split(',')[0].split('/')[1] + ', ' + position.updated.split(',')[0].split('/')[2]}</Text>
								</Td>
