...
import { Avatar, Badge, Button, Container, Divider, Flex, IconButton, Input, Select, Td, Text, Th, Tr, useDisclosure, useToast } from '@chakra-ui/react'
...
const ViewModal = ({ users, attendance: attAll, payroll }) => {
	...
	const sumDuration = (array) => {
		const totalMinutes = array.reduce((acc, curr) => acc + curr.duration.minutes, 0)
		const totalHours = array.reduce((acc, curr) => acc + curr.duration.hours, 0)
	...
	const sumAmounts = (array) => {
		const totalOvertime = array.reduce((acc, curr) => acc + curr.amount.overtime, 0)
		const totalNetPay = array.reduce((acc, curr) => acc + curr.amount.netpay, 0)
		const totalAmounts = {
	...
	const sumDeductions = (array) => {
		const totalDeductions = array.reduce((acc, curr) => acc + curr.amount, 0)
	...
	const editMutation = useMutation((data) => api.update('/payrolls', payroll._id, data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 3000,
				render: () => <Toast title="Success" description="Payroll successfully payed." />
			})
		}
	})
				...
				<Flex direction="column" gap={3}>
					<Flex justify="space-between" gap={3}>
						...
						{users
							...
							.map((user) => (
								<Text fontSize="sm" fontWeight="medium" color="accent-1" key={user._id}>
									{user.name}
								</Text>
							))}
					</Flex>
					...
					<Flex justify="space-between" gap={3}>
						...
						<Text fontSize="sm" fontWeight="medium" textTransform="capitalize" color="accent-1">
					...
					<Flex justify="space-between" gap={3}>
						...
						<Text fontSize="sm" fontWeight="medium" color="accent-1">
							- {isDeductionsFetched ? currency(sumDeductions(deductions)) : currency(0)}
						</Text>
					</Flex>
					...
					<Flex justify="space-between" gap={3}>
						...
						<Text fontSize="sm" fontWeight="medium" color="accent-1">
							{isDeductionsFetched ? currency(pay + overtime - sumDeductions(deductions)) : 'Loading...'}
						</Text>
					</Flex>
						...
						<Tr key={attendance._id}>
							<Td>
								<Flex align="center" gap={3}>
									...
									<Badge variant="tinted" colorScheme={attendance.timein.status.label !== 'Late' ? 'brand' : 'red'}>
							...
							<Td>
								{attendance.timeout.time !== '' ? (
									<Flex align="center" gap={3}>
										...
										<Badge variant="tinted" colorScheme={attendance.timeout.status.label !== 'Overtime' ? 'brand' : 'yellow'}>
							...
							<Td>
								<Text>{currency(attendance.amount.netpay)}</Text>
							</Td>
				...
				<Table
					...
					td={(deduction) => (
						<Tr key={deduction._id}>
							...
							<Td>
								<Text textTransform="uppercase">{deduction.title}</Text>
							</Td>
					...
					<Table
						...
						td={(payroll) => (
							<Tr key={payroll._id}>
								<Td>
									<Badge variant="tinted" colorScheme="brand">
										#{payroll._id.slice(15, 30).toUpperCase()}
									</Badge>
								</Td>
						...
						controls={(register) => (
							<Flex flex={1} justify="end" gap={3}>
								<Select placeholder="Select" size="lg" w="auto" {...register('status')}>
						...
						filters={(data, watch) => {
							return data.filter((data) => (watch('status') ? (watch('status') === 'Paid' ? true : false) === data.status : data))
						}}
