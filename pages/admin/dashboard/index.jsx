...
import { Avatar, Badge, Button, Container, Flex, Grid, GridItem, IconButton, Input, Td, Text, Tr } from '@chakra-ui/react'
...
const Dashboard = () => {
	...
	const tpt = isUsersFetched && isAttendanceFetched && watch('date') ? attendance.filter((att) => att.date === convert(watch('date'))).length : 0
	const te = isUsersFetched && isAttendanceFetched && watch('date') ? (attendance.filter((att) => att.date === convert(watch('date')))[0] ? attendance.filter((att) => att.date === convert(watch('date')))[0].extra.employees : 0) : 0
	const tat = te - tpt
	const pt = tpt && te ? ((100 * tpt) / te).toString().split('').slice(0, 5) : '00.00'
	...
	const exportToExcel = () => {
		...
		XLSX.writeFile(workbook, `dashboard-reports-${new Date().toLocaleString('en-US', { timeZone: 'Asia/Manila' })}.xlsx`)
	}
			...
			<Flex direction="column" gap={6}>
				<Flex justify="space-between" align="center" gap={6}>
					...
					<Flex align="center" gap={3}>
						...
						<Button size="lg" colorScheme="brand" leftIcon={<FiDownload size={16} />} onClick={exportToExcel}>
							Export
						</Button>
					</Flex>
				</Flex>
				...
				<Grid templateColumns="repeat(12, 1fr)" gap={6}>
					<Statistics date={date} today={watch('date') ? convert(watch('date')) : null} tpt={tpt} tat={tat} te={te} pt={pt} />
					...
					<GridItem colSpan={12}>
						<Card>
							<Table
								...
								th={['Id', 'Employee', 'Time In', 'Time Out', 'Date', '']}
								td={(attendance) => (
									<Tr key={attendance._id}>
										...
										{isUsersFetched &&
											users
												.filter((user) => user._id === attendance.user)
												.map((user) => (
													<Td key={user._id}>
														<Flex align="center" gap={3}>
								...
								filters={(data) => {
									return data.filter((data) => watch('date') && data.date === convert(watch('date')))
								}}
							/>
						</Card>
					</GridItem>
