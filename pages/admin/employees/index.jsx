...
const AddModal = ({ user, positions, schedules }) => {
	...
	const editMutation = useMutation((data) => api.update('/users/hire', user._id, data), {
		onSuccess: () => {
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					...
					<FormControl isInvalid={errors.position}>
						...
						<Select placeholder="Select" size="lg" textTransform="capitalize" {...register('position', { required: true })}>
							{positions.map((position) => (
								<chakra.option textTransform="capitalize" value={position._id} key={position._id}>
									{position.title}
								</chakra.option>
							))}
						</Select>
					...
					<FormControl isInvalid={errors.schedule}>
						...
						<Select placeholder="Select" size="lg" textTransform="capitalize" {...register('schedule', { required: true })}>
							{schedules.map((schedule) => (
								<chakra.option textTransform="capitalize" value={schedule._id} key={schedule._id}>
			...
			<Table
				...
				td={(user) => (
					<Tr key={user._id}>
						...
						<Td>
							<Flex justify="end">
								<AddModal user={user} positions={positions} schedules={schedules} />
							</Flex>
						</Td>
					</Tr>
				)}
		...
		<Container>
			<Flex direction="column" gap={6}>
				...
				<Card>
					<Table
						data={users}
						fetched={isUsersFetched && isPositionsFetched && isSchedulesFetched}
						th={['ID', 'Name', 'Position', 'Rate Per Hour', 'Schedule', '']}
						td={(user) => (
							<Tr key={user._id}>
								<Td>
								...
								<Td>
									<Text textTransform="capitalize">{user.position.title}</Text>
								</Td>
								...
								<Td>
									<Text>{user.schedule.timein + ' - ' + user.schedule.timeout}</Text>
								</Td>
						...
						controls={(register) => (
							<Flex flex={1} justify="end" align="center" gap={3}>
								<Select placeholder="Position" size="lg" w="auto" textTransform="capitalize" {...register('position')}>
									{isPositionsFetched &&
										positions.map((position) => (
											<chakra.option textTransform="capitalize" value={position.title} key={position._id}>
												{position.title}
											</chakra.option>
										))}
								</Select>

								<UsersModal users={users} isUsersFetched={isUsersFetched} positions={positions} schedules={schedules} />
							</Flex>
						)}
						filters={(data, watch) => {
							return data
								...
								.filter((data) => (watch('position') ? watch('position') === data.position.title : data))
						}}
