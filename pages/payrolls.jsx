...
import { Avatar, Badge, Container, Divider, Flex, IconButton, Td, Text, Th, Tr, useDisclosure } from '@chakra-ui/react'
					...
					<Table
						...
						td={(payroll) => (
							<Tr key={payroll._id}>
								...
								<Td>{payroll.status ? <Text>0</Text> : <Text>{isAttendanceFetched && attendance.filter((att) => att.user === payroll.user && !att.payed).length}</Text>}</Td>
								...
								<Td>
									<Flex justify="end">
										<ViewModal attendance={attendance} payroll={payroll} />
									</Flex>
								</Td>
							</Tr>
						)}
						filters={(data) => {
							return data.filter((data) => data.user === session.user.id)
						}}
					/>
				</Card>
			</Flex>
